 ...
 * drena y procesa el buffer al abrir/reanudar (ver `WalletIngestService`).
 *
 * Solo almacena título + texto + timestamp + paquete: no interpreta nada aquí
 * (el parseo y la creación del gasto ocurren en Dart).
 */
class WalletNotificationListenerService : NotificationListenerService() {
    ...
    companion object {
        ...
        fun readBuffer(service: android.content.Context, clear: Boolean): String {
            val prefs = service.getSharedPreferences(PREFS, MODE_PRIVATE)
            synchronized(LOCK) {
        ...
        fun setSources(context: android.content.Context, packages: List<String>) {
